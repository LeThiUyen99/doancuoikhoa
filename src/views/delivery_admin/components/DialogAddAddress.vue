    ...
    <el-row class="dialog-address">
      <el-form ref="ruleForm" :model="postForm" :rules="rules" class="demo-ruleForm">
        <el-form-item prop="golfer_id">
          ...
          <div class="address">
            <el-input ref="refVga" v-model="postForm.golfer_id" style="padding-right: 5px" :placeholder="$t('VGA')" />
            <el-button :loading="loadingSubmit" class="button-delivery" :autofocus="true" @click="searchUser">{{ $t('search') }}</el-button>
          </div>
        </el-form-item>
        <el-row style="display: flex">
          <el-form-item style="width: 100%; padding-right: 5px" :label="$t('full_name')" prop="name">
            <el-input v-model="postForm.name" :placeholder="$t('full_name')" autocomplete="off" />
          </el-form-item>
          <el-form-item style="width: 100%" :label="$t('phone')" prop="phone">
            <el-input ref="refPhone" v-model="postForm.phone" :placeholder="$t('phone')" autocomplete="off" />
          </el-form-item>
        </el-row>
        <el-row>
        ...
        <el-form-item :label="$t('specific_address')" :required="true" prop="specific_address">
          <el-input v-model="postForm.specific_address" :placeholder="$t('input_specific_address')" autocomplete="off" />
        </el-form-item>
      </el-form>
      <span style="float:right;">
        <el-button @click="handleClose">{{ $t('canel') }}</el-button>
        <el-button type="primary" :loading="loadingSubmit" @click="onSubmitForm('ruleForm')">{{ $t('add') }}</el-button>
      </span>
    </el-row>
  </el-dialog>
</template>
...
export default {
  ...
  watch: {
    ...
    provinceModel(province_id) {
      console.log('.......... lay thong tin tinh/thanh pho', province_id)
      this.resetDistrict()
      this.resetWards()
      if (province_id) {
  ...
  methods: {
    ...
    searchUser() {
      console.log('....................ma vga: ', this.vga)
      // if (!this.postForm.golfer_id || !this.postForm.golfer_id.length) {
      //   return this.$msgbox(i18n.t('vga_empty'))
      // }
      deliveryResource.get_golfer_info(this.postForm.golfer_id).then(res => {
    ...
    handleCreateAddress() {
      ...
      this.$msgbox(i18n.t('confirm_create_address')).then(() => {
        console.log('........tim kiem dia chi : ', JSON.stringify(city), JSON.stringify(district), JSON.stringify(wards))
        const body = {
        ...
        golferAddressResource.createAddress(body).then(response => {
          console.log('---------------------------create address 22 ', JSON.stringify(body))
          console.log('---------------------------create address thanh cong 22', JSON.stringify(response))
    ...
    handleClose(reload = false, info_address = null) {
      ...
      this.$emit('onClickButtonForm', { dialog: false, reload: reload, address: info_address, golfer: golfer })
    },
