    ...
    <el-form ref="ruleForm" :model="postForm" :rules="rules" class="demo-ruleForm">
      <el-row>
        <el-col :sm="18" class="pr-lg-5">
          <el-form-item :label="$t('shop_name')" prop="name">
            <el-input v-model="postForm.name" :placeholder="$t('shop_name')" />
          </el-form-item>
          ...
          <el-form-item :label="$t('address')" prop="address">
            <el-input v-model="postForm.address" :placeholder="$t('note_address')" />
          </el-form-item>
          <el-row>
            <el-col :sm="8" class="pr-lg-5">
              <el-form-item class="date" :label="$t('date_active')" prop="date_active">
                <el-date-picker
                  v-model="postForm.date_active"
                  type="datetime"
                  :placeholder="$t('date_active')"
                />
              </el-form-item>
            </el-col>
            <el-col :sm="8" class="pr-lg-5">
              <el-form-item class="date" :label="$t('date_limited')" prop="date_finish">
                <el-date-picker
                  v-model="postForm.date_finish"
                  type="datetime"
                  :placeholder="$t('date_limited')"
                />
              </el-form-item>
            </el-col>
          </el-row>
          ...
          <el-form-item :label="$t('show_on_home_shop')">
            <el-switch
              v-model="activeLogo"
        ...
        <el-col :sm="6">
          ...
          <el-form-item :label="$t('logo_menu')" prop="logo_menu">
            <div class="clearfix" />
...
export default {
  ...
  data() {
    return {
      dialog: { titleName: I18n.t('create_shop'), buttonName: I18n.t('add') },
      loadingDetail: false,
      loadingSubmit: false,
      loadingTable: false,
      rules: setRules(),
  ...
  watch: {
    ...
    activeLogo(value) {
      this.activeName = value ? I18n.t('yes') : I18n.t('no')
      this.postForm.active_show_menu = value ? 1 : 0
    },
    isAdd(value) {
      if (value) {
        this.dialog = Object.assign({}, { titleName: I18n.t('create_shop'), buttonName: I18n.t('add') })
        this.postForm = {}
        this.objectShopOrigin = {}
      } else {
        this.dialog = Object.assign({}, { titleName: I18n.t('update_shop'), buttonName: I18n.t('update') })
      }
    },
    showDialog(show) {
  ...
  methods: {
      ...
      this.$refs[formName].validate((validate) => {
        if (validate) {
          if (!this.provinceModel) {
            return this.$message.warning(I18n.t('select_city'))
          }
          if (!this.districtModel) {
            return this.$message.warning(I18n.t('select_district'))
          }
          if (!this.wardsModel) {
            return this.$message.warning(I18n.t('select_wards'))
          }
    ...
    handleSetInfoAddress() {
      ...
      if (!wards) {
        return this.$msgbox(I18n.t('wards_empty'))
      }
