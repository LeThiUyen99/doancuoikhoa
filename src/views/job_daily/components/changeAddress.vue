<template>
  <el-dialog
    :title="`${dialog.titleName} ${objectData.shop? objectData.shop.name: ''}`"
        ...
        <el-form :model="postForm" class="demo-ruleForm">
          <el-row>
            <el-col :sm="24" class="pr-lg-5">
              <el-form-item v-if="this.postForm.id" :label="$t('name')" prop="name">
    ...
    <el-row v-if="!isAddressShow" class="list-data-address">
      <el-col>
        ...
        <el-table v-loading="loadingAddress" :data="listAddress" border>
          ...
          <el-table-column :label="$t('shop_name')" prop="name" align="center">{{ objectData.shop? objectData.shop.name: name }}</el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      dialog: {
        titleName: I18n.t('supplier_address'),
  ...
  methods: {
    onShowDialogEdit(row) {
      ...
      this.activeAddress = row.is_default
    },
    onDeleteAddress(row) {
    ...
    choosenAddress(row) {
      this.shop_address_id = row.id
      this.delivery_id = this.deliveryId
      deliveryResource
        .changeAddress(this.delivery_id, this.shop_address_id)
        .then(res => {
          const { error, msg } = res
          if (error === 0) {
    ...
    handleAddAddress() {
      ...
      deliveryResource.createShopAddress(body).then(res => {
    ...
    handleUpdateAddress() {
      ...
      deliveryResource.updateAddress(body).then(res => {
        this.loadingSubmit = false
        const { error, insert, msg } = res
        body.address_id = insert
        if (error === 0) {
    ...
    requestAddress() {
      ...
      deliveryResource.addressList(this.shopId).then(res => {
