  ...
  >
    <el-form :model="objectData" class="saleForm">
      ...
      <el-form-item :label="$t('priority')" prop="priority">
        <el-input v-model="priority" :placeholder="$t('priority')" />
      </el-form-item>
    ...
    <div v-if="!objectData.id">
      <template>
        <el-radio v-model="radio" label="1" @change="change">{{ $t('product_name') }}</el-radio>
        ...
        >
          ...
          <el-table-column label="Action" align="center">
            <template slot-scope="scope">
              <el-button @click="selectedItem(scope.row)">Chọn</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      dialog: { titleName: i18n.t('add_product_priority'), buttonName: i18n.t('add') },
      is_add_action: false,
  ...
  watch: {
    ...
    isAdd(value) {
      ...
      if (value) {
        this.dialog = Object.assign({}, { titleName: i18n.t('add_product_priority'), buttonName: i18n.t('add') })
      } else {
        this.dialog = Object.assign({}, { titleName: i18n.t('update_product_priority'), buttonName: i18n.t('update') })
      }
    }
  },
  methods: {
    reset() {
      ...
      this.dialog = { titleName: i18n.t('add_product_priority'), buttonName: i18n.t('add') }
    ...
    onUpdatePriority() {
      this.$confirm(i18n.t('update_product_priority_confirm')).then(_ => {
        this.loadingSubmit = true
        saleResource.updatePriority(this.body).then(response => {
