      ...
      <el-form>
        <el-row>
          <div class="top-pro">
            ...
            <el-button style="margin-top: 10px; border-color: #0074FF; color: #0074FF" @click="addProduct">{{
              $t("choose_product_same_shop")
            }}</el-button>
          </div>
              ...
              <el-col :sm="12" class="pr-lg-5">
                <el-form-item>
                  ...
                  <div class="delivery">
                    <el-select
                      v-model="param"
                      :placeholder="$t('select_enter_cate')"
                    >
                      <el-option
                        v-for="par in params"
                        :key="par.id"
          ...
          <el-table :data="listProduct" border style="margin-top: 20px">
            ...
            >
              <template slot-scope="props">
                <el-link
                  type="primary"
                  @click="onShowDialogDetail(props.row)"
                >{{ `#${props.row.id}` }}</el-link>
              </template>
            </el-table-column>
            ...
            <el-table-column :label="$t('price_sale')" align="center">
              <template slot-scope="scope">{{
                $t("price_vnd").format(formatNumber(scope.row.price_sale))
              }}</template>
            </el-table-column>
            <el-table-column
              prop="shop_id"
        ...
        <el-form-item>
          ...
          <div class="delivery">
            <el-select
              v-model="golfer_address"
              :placeholder="$t('select_delivery_address')"
            >
              <el-option
                v-for="add in list_address"
                :key="add.id"
                :label="add.full_address"
                :value="add.id"
              />
            </el-select>
            <el-button class="button-delivery" @click="addAddress">{{
              $t("add_address")
            }}</el-button>
          </div>
        </el-form-item>
        <el-form-item>
          <div class="product-inf">
            ...
            <el-button
              ...
              class="update-deli"
              icon="el-icon-edit"
              @click="onShowDialogEdit(id_info)"
            />
            <div class="delivery">
              <label
                class="info-deli"
              >{{ $t("full_name") }}: {{ name_golfer }}</label>
            </div>
            <div class="delivery">
              <label
                class="info-deli"
              >{{ $t("phone") }}: {{ phone_golfer }}</label>
            </div>
          </div>
        </el-form-item>
        <el-row style="display: flex">
          ...
          <el-form-item style="width: 100%">
            ...
            <div class="delivery">
              ...
              >
                <el-option
                  v-for="transfer in list_transfer"
                  :key="transfer.MA_DV_CHINH"
                  :label="transfer.name"
                  :value="transfer.MA_DV_CHINH"
                />
              </el-select>
        ...
        <el-form-item>
          ...
          <div class="product-inf">
            <div class="delivery">
              <label
                class="info-deli"
              >{{ $t("delivery_date") }}: {{ date_transfer_bill }}</label>
            </div>
            <div class="delivery">
              <label
                class="info-deli"
              >{{ $t("salary_ship") }}:
                {{ $t("price_vnd").format(formatNumber(cod_price)) }}</label>
            </div>
            <div class="delivery">
              <label
                class="info-deli"
              >{{ $t("order_price") }}:
                {{ $t("price_vnd").format(formatNumber(bill_price)) }}</label>
            </div>
            <div class="delivery">
              <label
                class="info-deli"
              >{{ $t("total_money") }}:
                {{ $t("price_vnd").format(formatNumber(total_bill)) }}</label>
            </div>
          </div>
        </el-form-item>
      </el-form>
...
export default {
  ...
  watch: {
    ...
    listProduct() {
      ...
      const bills_price = this.listProduct.map(
        (d) => parseInt(d.price_sale) * parseInt(d.count)
      )
      this.bill_price =
        bills_price.length === 1
          ? bills_price[0]
          : bills_price.reduce((v1, v2) => v1 + v2)
    ...
    transfer_id() {
      const cod = this.list_transfer.find(
        (d) => d.MA_DV_CHINH === this.transfer_id
      )
  ...
  methods: {
    ...
    onShowDialogEdit(id_info) {
      ...
      this.idDelivery = id_info || this.list_address.find((d) => d.id === this.golfer_address).id
    ...
    onDelete(row) {
      this.$confirm(i18n.t('confirm_remove_product').format(row.name)).then(
        (_) => {
    ...
    addProduct() {
      ...
      this.getCheckProduct(() => {
        setTimeout(() => {
          ...
          this.product.param = this.product.params.find(
            (d) => d.id === this.param
          )
    ...
    pushListProduct(product) {
      const product_check = this.listProduct.find(
        (d) => d.id === product.id && d.param_id === product.param_id
      )
      if (!product_check) {
        this.listProduct.push(product)
      } else {
        product_check.count =
          parseInt(product_check.count) + parseInt(product.count)
        ...
        if (error === 0) {
          ...
          for (const param of data.params) {
            ...
            param_first = param_first
              ? param_first.replace('null', '').replace('undefined', '')
              : ''
            param_second = param_second
              ? param_second.replace('null', '').replace('undefined', '')
              : ''
            const names = []
    ...
    getCheckProduct(callback = null) {
      this.loadingTable = true
      productResource
        .checkProduct(this.param, this.count)
        .then((response) => {
          this.loadingTable = false
    ...
    requestTransport() {
      ...
      this.loadingTable = true
      const body = {
        golfer_address: this.list_address.find(
          (d) => d.id === this.golfer_address
        ),
          ...
          setTimeout(() => {
            ...
            if (error === 0) {
              ...
              this.list_transfer.forEach((d) => {
                d.name = `${d.TEN_DICHVU} - ${i18n.t('price')}: ${i18n
                  .t('price_vnd')
                  .format(formatNumber(d.GIA_CUOC))} - ${i18n.t('time')}: ${
                  d.THOI_GIAN
                }`
              })
            } else {
      ...
      this.$confirm(i18n.t('create_bill_confirm')).then(() => {
        const body = {
          ...
          method_payment: this.list_payment.find(
            (d) => d.id === this.method_payment_id
          ),
          address: this.list_address.find((d) => d.id === this.golfer_address),
          cod_ship: this.list_transfer.find(
            (d) => d.MA_DV_CHINH === this.transfer_id
          ),
          note: this.note,
          cod_ship_time: this.time_order.find(
            (d) => d.id === this.time_order_id
          )
        }
        this.loadingTable = true
        deliveryResource
